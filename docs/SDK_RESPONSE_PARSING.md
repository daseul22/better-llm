...
└────────────────────────┬────────────────────────────────────────┘
         ...
         │  3. UserMessage               │ ← 사용자 입력 (드물게)
         │  4. SystemMessage             │ ← 시스템 메타데이터
         └───────────────┬───────────────┘
                         │
                         ▼
         ┌───────────────────────────────┐
...
- **예시**:
  ...
  AssistantMessage(
      content=[
          ...
          ThinkingBlock(text="[사고 과정: 사용자가 인사를 건넴]")
      ],
      usage={
          'input_tokens': 120,
          'output_tokens': 45
...
**핵심 포인트**:
- ✅ **단계적 폴백**: 알려진 타입 → 알 수 없는 타입 순서로 처리
...
**핵심 포인트**:
- ✅ **순서 보장**: ResultMessage → AssistantMessage → 폴백
